...
f = open('/home/alexeedm/udevicex-scripts/data/viscosity/visc_backup.pckl', 'rb')
vm = pickle.load(f)
f.close()
...
for tag in vm.keys():
    ...
    f = open('/home/alexeedm/udevicex-scripts/data/visc_' + tag + '_backup.pckl', 'wb')
    pickle.dump(s, f)
    f.close()
...
#    if tag == '160.0_0.5' or tag == '160.0_1.0':
    g_fine = np.arange(20.0, 200, 1)
    plt.plot( g_fine, s(g_fine), zorder=5 )
    
    res = linregress(g_fine, s(g_fine))
    print(res.slope)
    
plt.axes().set_xlim([1, 150])
plt.axes().set_ylim([1, 150])
plt.legend()
plt.show()
