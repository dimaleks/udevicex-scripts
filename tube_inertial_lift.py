...
def dump_plots(positions, alldata, ref):
    ...
    plt.plot(ref[:,0], ref[:,1], "o", color="C0", label="C. Liu et al., Lab on a Chip (2015)", zorder=2, markeredgewidth=0.75, markeredgecolor='black', ms=3)
        
    for data, err, label, fmt, color in alldata:
        plt.errorbar(positions, data, yerr=err, fmt=fmt, color=color, label=label, markeredgewidth=0.75, markeredgecolor='black', ms=3, zorder=3)
...
def get_forces(case):
    ...
    for pos in positions:
        ...
        err_Cls.append(coefficient(5.0*math.sqrt(vy), rho, Um, r, R))
...
folder = "/home/alexeedm/extern/daint/project/alexeedm/focusing_rigid/case_50_0.15/"
